...
from decoder import Decoder,Classifier
import common
import metrics
import cv2
from torchvision import transforms
from contrastive import TripletLoss,MLPWithBN,NoisyModel,ContrastiveLoss
from noise_gan import GeneratorWithDynamicNoise,Scale
from abnormal import BidirectionalAttentionClassifier,ChannelAttention
...
class FUDet(torch.nn.Module):
    ...
    def load(
        ...
        **kwargs,
        ...
        if self.pre_proj > 0:
            ...
            # self.attention = BidirectionalAttentionClassifier(input_size=1536, hidden_size=1536)
            # self.attention.to(self.device)
            # self.proj_opt_a
            # 8ttention = torch.optim.AdamW(self.attention.parameters(), lr * .1)

            # self.channelattention1 = ChannelAttention(in_planes=512)
            # self.channelattention2 = ChannelAttention(in_planes=1024)
            # self.proj_opt_channel_attention1 = torch.optim.AdamW(self.channelattention1.parameters(), lr * .1)
            # self.proj_opt_channel_attention2 = torch.optim.AdamW(self.channelattention2.parameters(), lr * .1)
            # self.channelattention2.to(self.device)
            # self.channelattention1.to(self.device)
        ...
        self.proj_opt_decoder = torch.optim.AdamW(self.decoder.parameters(), lr * .1)
        self.model_dir = ""
        self.dataset_name = ""
        self.tau = 1
        self.logger = None
        ...
        self.contrastiveloss.to(self.device)
        self.MLPBN = MLPWithBN(1536, 1536,1536)
        self.MLPBN.to(self.device)
        self.noisegan=GeneratorWithDynamicNoise(feature_dim=1536)
        self.noisegan.to(self.device)
        self.weight = Scale(scale=10)
        self.linear=nn.Linear(4232,2700)
    ...
    def gan(self, true_feats, ffake_feats):

        scores = self.discriminator(torch.cat([true_feats, ffake_feats]))
        true_scores = scores[:len(true_feats)]
        fake_scores = scores[len(ffake_feats):]
        th = self.dsc_margin
    ...
    def _embed(self, images, detach=True, provide_patch_shapes=False, evaluation=False):
        ...
        if not evaluation and self.train_backbone:
            ...
            with torch.no_grad():
                features,chonggou_features = self.forward_modules["feature_aggregator"](images)
                # self.forward_modules["feature_aggregator"].layers_to_extract_from=('maxpool','layer1','layer2','layer3','layer4')
                # chonggou_features =self.forward_modules["feature_aggregator"](images)
        # features['layer2']=self.channelattention1(features['layer2'])
        # features['layer3']=self.channelattention2(features['layer3'])
        ...
        # features = features[1]+self.conv1(features[0])
        features_ = torch.cat((features[0], features[1]), dim=1)
        ...
        return features, patch_shapes,chonggou_features, features_
    ...
    def _evaluate(self, test_data, scores, segmentations, features, labels_gt, masks_gt):
        ...
        return auroc, full_pixel_auroc, pro,norm_segmentations
    ...
    def train(self, training_data, test_data):
        ...
        if os.path.exists(ckpt_path):
            ...
            scores, segmentations, features, labels_gt, masks_gt,name = self.predict(test_data)
            auroc, full_pixel_auroc, anomaly_pixel_auroc,norm_segmentations = self._evaluate(test_data, scores, segmentations, features, labels_gt, masks_gt)


                # segmentations_list = norm_segmentations.copy()
                # for k, seg in enumerate(segmentations_list):
                #     for i, value_i in enumerate(seg):
                #         for j, value_j in enumerate(value_i):
                #             if value_j > 0.1:
                #                 segmentations[k][i][j] = 255
                #             else:
                #                 segmentations[k][i][j] = 0
                #     cv2.imwrite('outputs' + '/' + name[k].split('/')[-4] + '_' + name[k].split('/')[-2] + '_' +
                #                 name[k].split('/')[-1], segmentations[k])
        ...
        def update_state_dict(d):
            ...
            if self.pre_proj > 0:
                ...
                # state_dict["attention"] = OrderedDict({
                #     k: v.detach().cpu()
        ...
        for i_mepoch in range(self.meta_epochs):
            ...
            auroc, full_pixel_auroc, pro,norm_segmentations = self._evaluate(test_data, scores, segmentations, features, labels_gt, masks_gt)
            ...
            #         # state_dict = OrderedDict({k:v.detach().cpu() for k, v in self.state_dict().items()})
                ...
                if full_pixel_auroc > best_record[1]:
                    ...
                    update_state_dict(state_dict)
                    # state_dict = OrderedDict({k:v.detach().cpu() for k, v in self.state_dict().items()})
                elif full_pixel_auroc == best_record[1] and auroc > best_record[0]:
                    best_record[1] = full_pixel_auroc
                    best_record[2] = pro
                    update_state_dict(state_dict)
                    # state_dict = OrderedDict({k:v.detach().cpu() for k, v in self.state_dict().items()})
            ...
            for i_epoch in range(self.gan_epochs):
                ...
                for data_item in input_data:
                    ...
                    if self.pre_proj > 0:
                        ...
                        # self.proj_opt_attention.zero_grad()
                        # self.proj_opt_channel_attention1.zero_grad()
                        # self.proj_opt_channel_attention2.zero_grad()
                    # self.dec_opt.zero_grad()
                    self.proj_opt_decoder.zero_grad()
                    i_iter += 1
                    img = data_item["image"]
                    img = img.to(torch.float).to(self.device)

                    fake_img = data_item["fake_image"]
                    fake_img = fake_img.to(torch.float).to(self.device)

                    if self.pre_proj > 0:
                        # true_feats = self.attention(self._embed(img, evaluation=False)[0])
                        # fakegan_feats = self.attention(self._embed(fake_img, evaluation=False)[0])
                        true_feats = self.pre_projection(self._embed(img, evaluation=False)[0])
                        fakegan_feats = self.pre_projection(self._embed(fake_img, evaluation=False)[0])
                        _,_,_,features=self._embed(img, evaluation=False)
                        features_decoder=self.linear(true_feats.reshape(1536,-1)).reshape(img.shape[0],256,45,45)
                        moni_img=self.decoder(features_decoder)

                    else:
                        true_feats = self._embed(img, evaluation=False)[0]
                    # noise_idxs=torch.rand(torch.Size([true_feats.shape[0]]))
                    ...
                    noise = self.noisegan(features).to(self.device)
                    #
                    # noise=self.noisegan(true_feats).to(self.device)
                    # noise = (noise * noise_one_hot.unsqueeze(-1)).sum(1)
                    noise = noise.sum(1)
                    # fake_feats=self.noisegan(true_feats)
                    fake_feats = true_feats + noise
                    features_decoder_fake =self.linear(fake_feats.reshape(1536,-1)).reshape(img.shape[0], 256, 45, 45)
                    moni_img_nosie = self.decoder(features_decoder_fake)
                    ffake_feats=self.MLPBN(true_feats)
                    # contrastive_loss=self.contrastiveloss(true_feats,fake_feats,torch.tensor([0]).to(self.device))
                    contrastive_loss = self.TripletLoss(fakegan_feats, ffake_feats, true_feats)
                    gan_loss=(self.gan(true_feats,ffake_feats)+torch.sum(noise)/self.weight(3))/2

                    modules = list(self.backbone.children())[:-1]
                    backbone_modified = nn.Sequential(*modules)
                    # img_zhengze=img/255.0
                    # normalize = transforms.Normalize(mean = [0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225])
                    # normalized=normalize(img_zhengze)
                    chonggou_loss=self.criterion(moni_img,img)
                    chonggou_loss_fake=self.criterion(moni_img_nosie,img)
                    classfier_out = self.classfier(backbone_modified(moni_img),backbone_modified(moni_img_nosie))
                    classfier_loss = self.criterion(classfier_out, torch.cat((torch.ones(8), torch.zeros((8)))).cuda(1))
                    ...
                    #+chonggou_loss_fake+chonggou_loss
                    loss = true_loss.mean() + fake_loss.mean()+contrastive_loss+gan_loss+chonggou_loss+chonggou_loss_fake+classfier_loss
                    self.logger.logger.add_scalar("loss", loss, self.logger.g_iter)
                    self.logger.step()

                    loss.backward()

                    self.proj_opt_decoder.step()


                    if self.pre_proj > 0:
                        self.proj_opt.step()
                        # self.proj_opt_attention.step()
                        # self.proj_opt_channel_attention1.step()
                        # self.proj_opt_channel_attention2.step()
                    if self.train_backbone:
    ...
    def _predict(self, images):
        ...
        with torch.no_grad():
            features, patch_shapes,_,_ = self._embed(images,
