...
_DATASETS = {
    ...
    "MFL": ["datasets.MFL", "MFLDataset"],
    "MFL_fake": ["datasets.MFL_fake", "MFLfakeDataset"]
}
...
def net(
    ...
    def get_simplenet(input_shape, device):
        ...
        for backbone_name, layers_to_extract_from in zip(
            ...
            simplenet_inst = FUDet.FUDet(device)
            simplenet_inst.load(
                backbone=backbone,
...
@click.argument("data_path", type=click.Path(exists=True, file_okay=False),default='MFL')
@click.option("--subdatasets", "-d", multiple=True, type=str, default=['MFL'])
